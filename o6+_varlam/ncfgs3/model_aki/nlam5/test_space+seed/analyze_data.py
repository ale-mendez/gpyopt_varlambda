...
#    plt.xlim(erp_min*0.9,erp_max*1.1)
    ...
    plt.savefig("Jmin.eps",transparent=True)
    #plt.show()
    return
...
#    plt.xlim(erp_min*0.9,erp_max*1.1)
    ...
    ax.xaxis.set_minor_locator(mticker.AutoMinorLocator(n=5));
    plt.subplots_adjust(left=sp_left,right=sp_right,bottom=sp_bottom,top=sp_top)   # <--
    plt.savefig("imin.eps",transparent=True)
    #plt.show()
    return
...
def plot_minspace(folder,Jmin,nlam):
    ...
    if nlam<=3:
        ...
        plt.savefig("minspace.eps",transparent=True)
...
#             ax.xaxis.set_major_locator(mticker.MultipleLocator(0.05));
        ...
        plt.subplots_adjust(left=0.15,right=0.88,bottom=sp_bottom,top=sp_top)   # <--
        plt.savefig("minspace1.eps",transparent=True)
...
#             ax.xaxis.set_major_locator(mticker.MultipleLocator(0.05));
        ...
        plt.subplots_adjust(left=0.15,right=0.88,bottom=sp_bottom,top=sp_top)   # <--
        plt.savefig("minspace2.eps",transparent=True)
    #plt.show()
    return

def plot_akierp(erp):
    global folder,ypad,xpad,akierp_min1,akierp_max1,akierp_min2,akierp_max2,ntran
    if ntran>6: ntran=6
    if ntran <= 3:
        nrow=1
        ncol=1 
    if (ntran > 3) & (ntran <= 6): 
        nrow=2
        ncol=1
    fig,axs=plt.subplots(nrows=nrow,ncols=ncol,sharey=True,figsize=(12,7))
    if ntran<=3:
        if ntran>=1: tran1,=axs.plot(erp[0]['er%'],erp[0].index,c=colors[0],marker=symbs[0],linestyle='None',markersize=12,label="1") 
        if ntran>=2: tran2,=axs.plot(erp[1]['er%'],erp[1].index,c=colors[1],marker=symbs[1],linestyle='None',markersize=12,label="2")
        if ntran>=3: tran3,=axs.plot(erp[2]['er%'],erp[2].index,c=colors[2],marker=symbs[2],linestyle='None',markersize=12,label="3")
    else:
        if ntran>=1: tran1,=axs[0].plot(erp[0]['er%'],erp[0].index,c=colors[0],marker=symbs[0],linestyle='None',markersize=12,label="1")
        if ntran>=2: tran2,=axs[0].plot(erp[1]['er%'],erp[1].index,c=colors[1],marker=symbs[1],linestyle='None',markersize=12,label="2")
        if ntran>=3: tran3,=axs[0].plot(erp[2]['er%'],erp[2].index,c=colors[2],marker=symbs[2],linestyle='None',markersize=12,label="3")
        if ntran>=4: tran4,=axs[1].plot(erp[3]['er%'],erp[3].index,c=colors[3],marker=symbs[3],linestyle='None',markersize=12,label="4")
        if ntran>=5: tran5,=axs[1].plot(erp[4]['er%'],erp[4].index,c=colors[4],marker=symbs[4],linestyle='None',markersize=12,label="5")
        if ntran==6: tran6,=axs[1].plot(erp[5]['er%'],erp[5].index,c=colors[5],marker=symbs[5],linestyle='None',markersize=12,label="6")
    axs = np.array(axs)
    for ax in axs.reshape(-1):
        ax.set_ylabel("N",fontsize=35,labelpad=ypad)
        ax.set_xlabel(r"Error relativo \%",fontsize=35,labelpad=xpad,position=(1,0))
        ax.xaxis.set_minor_locator(mticker.AutoMinorLocator(n=5))
...
#    plt.xlim(akierp_min2,akierp_max2)
    ...
    for ax in fig.get_axes():
        ax.label_outer()
    fig.subplots_adjust(hspace=0.05, wspace=0.05)
    plt.legend(bbox_to_anchor=(1,1),loc='upper left',fontsize=25,ncol=1,handlelength=0.2,frameon=False)
    plt.subplots_adjust(left=0.11,right=0.75,bottom=sp_bottom,top=sp_top)   # <--
    plt.savefig("akierp.eps",transparent=True)
#    plt.show()
    return
