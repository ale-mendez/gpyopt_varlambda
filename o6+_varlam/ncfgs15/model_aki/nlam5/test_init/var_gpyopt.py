...
def lamkernel(rbf,stdperiodic):
    ...
    if rbf==0 and stdperiodic==1: kernel=GPy.kern.StdPeriodic(input_dim=nlamvar,variance=varf,period=None,lengthscale=None,ARD1=ARD,ARD2=ARD)
...
def run_bo(iseed,space,kernel,function,initer,initmap,maxevals,varf,lf,noise_var,exact_feval,optimize_restarts,ARD,xi):
    ...
    if init_design==1:
       Xinit=GPyOpt.experiment_design.initial_design(initmap,dspace,initer)
       Yinit=None
    else:
       Xinit=Xinp
       Yinit=Yinp
    ...
    myBopt=GPyOpt.methods.ModularBayesianOptimization(model,dspace,objective,acquisition,evaluator,
                                                      X_init=Xinit,Y_init=Yinit,
                                                      normalize_Y=True,model_update_interval=1)
...
if ioptmod==1:
    ...
    kernel=lamkernel(rbf=0,stdperiodic=1)
# inquire if initial_design exists:
    finit="init_design.dat"
    init_design=1
    iuse='n'
    icreate='n'
    ...
    if init_input=='yes':
        try:
            f = open(finit)
            print(" Initial design exists. ")
            iuse = input(" Use it? y/n: ")
#            iuse='y'
        except IOError:
            print(" Initial design does not exists. ")
            icreate = input(" Create now? y/n: ")  # Python 3
            if icreate=='y':
                init_design=1
                print(' >>> I am creatting new initial data now. ')
                maxevals=0
            else:
        ...
        if iuse=='y':
            init_design=0
            pd_init=pd.read_csv(finit,sep='\t',header='infer')
            ncols=len(pd_init.columns)
            nrows=len(pd_init)
            lxinp=[]
            lyinp=[]
            for i in range(nrows):
                xdum=np.array(pd_init.loc[i][2:].tolist())
                ydum=pd_init.loc[i][1].tolist()
                ydum=[ydum]
                vydum=np.array(ydum)
                lxinp.append(xdum)
                lyinp.append(vydum)
    ...
    if (init_input=='yes') and (icreate=='y'):
        os.system('mv '+filename+'.dat init_design.dat') 
        os.system('rm '+filename+'.*')
        print(" Re run code now.")
