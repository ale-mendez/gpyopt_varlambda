...
global nbins,Nmin,Nmax,amin,amax,rmin,rmax,jmin,jmax,imin,imax,initer,totevals,ymajor_imin,yminor_imin
...
def plot_imin(folder,iJmin):
    global jmin,jmax,imin,imax,initer,totevals,ymajor_imin,yminor_imin
    fig=plt.figure(figsize=(8,8))
    ax=plt.subplot(111)
...
if ilatin==1:
    ...
    print(folder)
    Jminmax=max(Jmin_lat['erp'])
    Jminmin=min(Jmin_lat['erp'])
    print("J max=",Jminmax)
    print("J min=",Jminmin)
    lJmin_lat=Jmin_lat['erp'].tolist()
    iminmin=lJmin_lat.index(Jminmin)
    alpha_latmin=alpha_lat.iloc[iminmin].tolist()
    rho_latmin=rho_lat.iloc[iminmin].tolist()
    print("alpha=",alpha_latmin[1:])
    print("rho=",rho_latmin[1:])
...
if irandom==1:
    ...
    print(folder)
    print("max=",max(Jmin_rand['erp']))
    print("min=",min(Jmin_rand['erp']))
