...
def data_input():
    global gridtype, cfgs, maxevals, mtype, aftype, afweight, gridtype, igrid, initer, ipol
    global nlamvar, maxlam, minlam, mfunc, ifun, minst, imin, nener, wi, igi, iseed, initmap
    print("\n===>>> Input parameters <<<===\n")
...
def mypolspace(alpha_min,alpha_max,rho_min,rho_max,npolvar):
    space=[]
    for i in range(npolvar):
        apol=str(i)
...
def run_mybo_pol(iseed,space,kernel,initer,initmap,maxevals,varf,lf,noise_var,exact_feval,optimize_restarts,ARD,xi):
    seed(iseed)
