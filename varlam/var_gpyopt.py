...
def data_input():
    global gridtype, cfgs, maxevals, mtype, aftype, afweight, gridtype, igrid, initer, ipol
    global nlamvar, maxlam, minlam, mfunc, ifun, minst, imin, nener, ntran, wi, igi, iseed, initmap, cost
    print('\n===>>> Input parameters <<<===\n')
...
def print_input():
    global gridtype, cfgs, maxevals, mtype, aftype, afweight, gridtype, igrid, initer, ipol
    global nlamvar, maxlam, minlam, mfunc, ifun, minst, imin, nener, wi, igi, iseed, initmap, cost
...
def loss_sumaki():
    ...
    facc=autovarlambda.faccbck.facce
    loss=0.0
    for i in range(imin,ntran): # consider a certain number of transitions
        ...
        if ifun==1: erp=erp*erp
        loss=loss+erp
#        print(' {} {:.3e} {:.3e} {:.4f} {:.4f}'.format(i,vakie[i],vakic[i],facc[i],erp))
    return loss
